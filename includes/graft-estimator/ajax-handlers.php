...
if (!function_exists('shec_guess_tip_from_question')) {
  /**
   * نکتهٔ کوتاه و کاربردی فقط بر اساس متن سؤال و پاسخ بله/خیر (بدون AI)
   * ورودی‌ها: $q = متن سؤال، $a_label = «بله»/«خیر»
   */
  function shec_guess_tip_from_question($q, $a_label){
    $q   = (string)$q;
    $yes = (trim($a_label) === 'بله');

    // --- پوست سر / عفونت‌ها ---
    if (preg_match('~عفونت|التهاب|چرک|ترشح|زخم(?:\s*فعال)?|قرمزی شدید|تاول|فولیکولیت|پسوریازیس|قارچ|سبورئیک|درماتیت~u', $q)) {
      return $yes
        ? 'اگر ضایعهٔ فعال دارید، ابتدا درمان کامل و فروکش التهاب؛ سپس زمان کاشت را می‌چینیم تا ریسک خونریزی/عفونت و افت بقای گرافت پایین بیاید.'
        : 'در نبود ضایعهٔ فعال، می‌توان برنامه‌ریزی کاشت را انجام داد؛ هر علامت غیرعادی (خارش/ترشح/قرمزی) را سریع اطلاع دهید.';
    }

    // --- ضدانعقادهای نام‌برده / انعقاد ---
    if (preg_match('~وارفارین|warfarin~iu', $q)) {
      return 'وارفارین نیاز به تنظیم INR و برنامهٔ قطع/bridging با تأیید پزشک معالج دارد؛ خودسرانه تغییر ندهید.';
    }
    if (preg_match('~هپارین|enoxaparin|کلیکسان|heparin|LMWH~iu', $q)) {
      return 'هپارین/LMWH باید طبق نظر پزشک معالج زمان‌بندی شود؛ دربارهٔ آخرین دوزِ پیش از عمل هماهنگ می‌کنیم.';
    }
    if (preg_match('~ریواروکسابان|rivaroxaban|اپیکسابان|apixaban|دابیگاتران|dabigatran~iu', $q)) {
      return 'DOACها باید بر اساس ریسک فردی و عملکرد کلیه موقتاً مدیریت شوند؛ قطع/ادامه فقط با دستور پزشک.';
    }
    if (preg_match('~آسپرین( بچه)?|aspirin|کلوپیدوگرل|clopidogrel~iu', $q)) {
      return 'آسپرین/کلوپیدوگرل خونریزی را افزایش می‌دهد؛ تصمیم دربارهٔ قطع/ادامه فقط با کاردیولوژیست و پزشک معالج.';
    }
    if (preg_match('~انعقاد|رقیق[ -]?کننده~u', $q)) {
      return 'اگر رقیق‌کننده مصرف می‌کنید، هر تغییر باید با تأیید پزشک باشد؛ ممکن است جایگزینی موقت لازم شود.';
    }

    // --- قلبی/عروقی ---
    if (preg_match('~قلب|عروقی|سکته|پرفشاری|کاردیو|cardio~iu', $q)) {
      return 'در سابقهٔ قلبی، کنترل فشار و تأیید پزشک ضروری است؛ در وضعیت پایدار، کاشت با پایش نزدیک قابل انجام است.';
    }

    // --- تیروئید ---
    if (preg_match('~تیروئید|TSH|hypo|hyper~iu', $q)) {
      return $yes
        ? 'TSH/Free T4 را به تعادل برسانید و نامهٔ پزشک ارائه شود؛ عدم تعادل هورمونی رشد مو را مختل می‌کند.'
        : 'خوب است؛ غربالگری پیش‌عمل برای اطمینان از تعادل هورمونی انجام می‌شود.';
    }

    // --- نقص ایمنی / شیمی‌درمانی / خودایمنی ---
    if (preg_match('~ایمنی|HIV|ایدز|شیمی[ -]?درمانی|سرکوب\s*ایمنی~u', $q)) {
      return 'به‌علت ریسک عفونت و ترمیم کند، تصمیم‌گیری منوط به ارزیابی تخصصی و تأیید پزشک معالج است.';
    }
    if (preg_match('~خودایمنی|لوپوس|areata|آلوپسی\s*آره ?آتا|پسوریازیس~iu', $q)) {
      return 'بیماری خودایمنیِ فعال می‌تواند پذیرش گرافت را کم کند؛ در صورت کنترل و عدم درگیری وسیع پوست سر، امکان برنامه‌ریزی وجود دارد.';
    }

    // --- داروهای مرتبط با مو ---
    if (preg_match('~ایزوترتینوئین|راکوتان|اکوتان|isotretinoin|accutane~iu', $q)) {
      return 'ایزوترتینوئین می‌تواند ترمیم زخم را مختل کند؛ دربارهٔ فاصلهٔ ایمن از آخرین دوز با پزشک معالج هماهنگ می‌کنیم.';
    }
    if (preg_match('~فیناستراید|finasteride|دوتاستراید|dutasteride~iu', $q)) {
      return $yes
        ? 'درمان هورمونی می‌تواند کمک‌کننده باشد؛ دربارهٔ تداوم/تنظیم دوز پیرامون روز عمل طبق پروتکل کلینیک راهنمایی می‌شوید.'
        : 'در صورت صلاح‌دید پزشک، درمان نگه‌دارندهٔ هورمونی می‌تواند به تثبیت ریزش کمک کند؛ در جلسهٔ حضوری بررسی می‌کنیم.';
    }
    if (preg_match('~ماینوکسیدیل|minoxidil~iu', $q)) {
      return 'ماینوکسیدیل موضعی معمولاً ۳–۵ روز قبل قطع و ۷–۱۰ روز بعد از عمل شروع می‌شود؛ طبق دستور کلینیک عمل کنید.';
    }

    // --- NSAID / مسکن‌ها ---
    if (preg_match('~ایبوپروفن|ژلوفن|ناپروکسن|دیکلوفناک|NSAID~iu', $q)) {
      return 'NSAIDها می‌توانند خونریزی را کمی افزایش دهند؛ جایگزینی مسکن طبق پروتکل کلینیک انجام می‌شود.';
    }

    // --- سبک زندگی: سیگار، خواب، استرس، ورزش ---
    if (preg_match('~سیگار|قلیان|دخانیات|نیکوتین~u', $q)) {
      return $yes
        ? 'اگر مصرف بالاست، از ۱۰ روز قبل تا ۷ روز بعدِ کاشت کاملاً قطع کنید؛ نیکوتین خون‌رسانی را کم و ترمیم را کند می‌کند.'
        : 'عالی؛ لطفاً ۷–۱۰ روز قبلِ کاشت تا یک هفته بعد هم کاملاً ترک کنید تا بقای گرافت‌ها بهتر شود.';
    }
    if (preg_match('~خواب|بی[ -]?خوابی|کیفیت\s*خواب~u', $q)) {
      return $yes
        ? 'بهداشت خواب: ساعت منظم، قطع کافئین بعدازظهر و کاهش نور آبی شب؛ خواب کافی التهاب را کم و ترمیم را سریع‌تر می‌کند.'
        : 'خوب است؛ شبِ قبل از کاشت خواب کامل داشته باشید و الگوی منظم را حفظ کنید.';
    }
    if (preg_match('~استرس|اضطراب|فشار\s*روان~u', $q)) {
      return $yes
        ? 'روزانه ۱۰–۱۵ دقیقه تنفس عمیق/پیاده‌روی سبک اضافه کنید؛ استرس بالا ریزش تلوژن را تشدید می‌کند.'
        : 'عالی؛ همین روال را ادامه دهید. چند دقیقه تمرین آرام‌سازی روزانه هم به بهبود ترمیم کمک می‌کند.';
    }
    if (preg_match('~ورزش|فعالیت\s*بدنی|تمرین|تعریق~u', $q)) {
      return $yes
        ? 'تا ۷۲ ساعت بعد از کاشت از ورزش شدید، سونا و تعریق زیاد پرهیز کنید؛ سپس فعالیت سبک را تدریجی شروع کنید.'
        : 'پس از کاشت، پیاده‌روی سبک از روزهای اول مجاز است اما فعالیت سنگین تا ۷۲ ساعت ممنوع است.';
    }

    // --- درد/بی‌حسی ---
    if (preg_match('~درد|بی[ -]?حسی|ناراحتی(?:\s*حین)?\s*عمل|حساسیت\s*به\s*بی‌حسی~u', $q)) {
      return $yes
        ? 'حین عمل بی‌حسی موضعی داریم و سطح راحتی پیوسته تنظیم می‌شود؛ بعد از عمل هم مسکن استاندارد کافی است.'
        : 'نگران نباشید؛ بی‌حسی موضعی درد را کنترل می‌کند و امکان تنظیم حین کار وجود دارد.';
    }

    // --- نقاهت/زمان نتیجه ---
    if (preg_match('~نقاهت|بهبودی|چند\s*ماه|چقدر\s*طول|timeline~iu', $q)) {
      return 'نتیجه مرحله‌ای است: شوک‌ریزش هفتهٔ ۲–۳، شروع رشد ۳–۴ ماه، پرپشتی ۶–۹ ماه و تثبیت حوالی ۱۲ ماه.';
    }

    // --- روند/شدت ریزش ---
    if (preg_match('~بدتر\s*شده|شدت\s*ریزش|افزایش\s*ریزش|اخیراً|اخیرًا~u', $q)) {
      return $yes
        ? 'با توجه به بدترشدن اخیر، تصمیم درمانی را عقب نیندازید و درمان نگه‌دارنده را هم‌زمان شروع کنید.'
        : 'خوب است که روند بدتر نشده؛ درمان نگه‌دارنده را برای تثبیت وضعیت ادامه دهید.';
    }

    // --- شرایط شغلی/محیطی ---
    if (preg_match('~کار\s*(?:سنگین|سخت)|وزنه|ساختمانی|گرد\s*و\s*غبار|کارگاه|کوره|سیمان|کاشی~u', $q)) {
      return 'در محیط‌های پرگردوغبار/کار سنگین، تا ۷۲ ساعت اول از تعریق و تماس محل کاشت محافظت کنید؛ از کلاه تمیز و بدون فشار استفاده شود.';
    }

    // --- پیش‌فرض امن و غیرکِلیشه‌ای ---
    return $yes
      ? 'با مدیریت عوامل سبک زندگی و تکمیل بررسی‌های پیش‌عمل، ریسک‌ها کاهش و کیفیت ترمیم بهتر می‌شود.'
      : 'روال فعلی مناسب است؛ همان را حفظ کنید و دستورات مراقبتی پیش و پس از عمل را دقیق اجرا کنید.';
  }
}
...
<div id="shec-result-token" class="shec-result-root">
  ...
  <div id="proposal-pdf-root" class="proposal-container">
    ...
    <div class="actions mt-3" style="margin-bottom:15px; display:flex; gap:12px; flex-wrap:wrap; justify-content:center;">
      <button id="reset-form" style="margin-bottom:15px;" data-reset-href="<?php echo esc_attr($calc_url); ?>" class="btn btn-danger">شروع مجدد</button>
      <button id="download-pdf" style="margin-bottom:15px;" class="btn btn-primary">دانلود PDF</button>
    </div>
  </div>
</div>
<?php
